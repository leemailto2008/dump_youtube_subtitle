...
class YouTubeSubExporter:
    def __init__(self, max_concurrent: int = 1):
        # 由於目前環境已出現 429 錯誤，並發設為 1 並增加隨機延遲是較安全的做法
    ...
    def get_video_infos(self, url: str) -> Tuple[List[VideoInfo], str]:
        ...
        try:
            ...
            elif c_match and not v_match:
                handle = c_match.group(1) or "Channel"
                print(f"[*] 解析頻道: {handle}")
    ...
    async def fetch_transcript(self, video_id: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                # 獲取清單
                method = getattr(api, 'list', getattr(api, 'list_transcripts', None))
                if not method: raise AttributeError("No list method")
                
                ts_list = await asyncio.to_thread(method, video_id)
            ...
            lines = []
            # 適配特定環境下的 Snippet 物件 (屬性：start, text)
    ...
    async def process_video(self, info: VideoInfo, target_dir: str):
        async with self.semaphore:
            ...
            if not transcript:
                print(f"[-] 無法抓取字幕 (可能是 429 限制或無字幕): {info.title}")
                return
...
async def main():
    if len(sys.argv) < 2:
        print("用法: python ytsub_ag02.py \"<URL>\" [自訂資料夾]")
        return

    input_url = sys.argv[1]
    user_path = sys.argv[2] if len(sys.argv) > 2 else None
    
    # 預設並發設為 1 以降低封鎖風險，若有需要可自行改回 5
    exporter = YouTubeSubExporter(max_concurrent=1)
    ...
    print(f"[*] 待處理影片共 {len(video_list)} 部")
